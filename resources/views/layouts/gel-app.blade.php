...
<body>
  ...
  <header class="sec-topbar">
    <div class="sec-topbar-brand">GEL <small>Business</small></div>

    {{-- Sélecteur d'entreprise --}}
    @php
      $user = auth()->user();
      
      // Récupérer les entreprises où l'utilisateur a une affectation active
      ...
      <div class="client-dropdown" id="clientDropdown">
        @forelse($entreprises as $entreprise)
          <form method="POST" action="{{ route('dashboard.switch-entreprise') }}" style="margin:0;">
            ...
            <button type="submit"
              class="client-dd-item w-100 border-0 text-start {{ $activeEntreprise?->id == $entreprise->id ? 'active' : '' }}">
    ...
    <div class="sec-topbar-right">
      ...
      <div style="position:relative;">
        ...
        <div class="sec-dropdown" id="quickActionsDd" style="right:0;top:calc(100% + 6px); width:200px; padding:8px 0;">
          <a href="#" class="sec-dd-item text-decoration-none"><i
          ...
          <a href="#" class="sec-dd-item text-decoration-none"
      ...
      <div style="position:relative;">
        ...
        <div class="sec-dropdown" id="userDd" style="right:0;top:calc(100% + 6px);">
          <div style="padding:10px 14px;border-bottom:1px solid var(--sec-border);">
            <div style="font-weight:600;font-size:13px;">{{ auth()->user()?->nom ?? 'Utilisateur' }}</div>
            <div style="font-size:11px;color:var(--sec-text-muted);">{{ auth()->user()?->email }}</div>
            <div style="font-size:11px;color:var(--sec-primary);font-weight:600;margin-top:2px;">{{ ucfirst(auth()->user()->role ?? 'Utilisateur') }}
            </div>
          </div>
          @if(in_array(auth()->user()->role ?? '', ['admin','super_admin','director','dirigeant']))
            <a href="#" class="sec-dd-item" style="color:var(--sec-info); font-weight:600; background:#EFF6FF;">
  ...
  <aside class="sec-sidebar">
    <div class="sec-sidebar-brand">
      <div class="sec-sidebar-icon">G</div>
      <div>
        <div class="sec-sidebar-title">GEL Cabinet</div>
        <div class="sec-sidebar-sub">Espace Gestion</div>
      </div>
    </div>
    ...
    <ul class="sec-nav">
      ...
      @if(isset($activeEntreprise) && $activeEntreprise)
        ...
        <li>
          <a href="{{ route('super-admin.entreprises.index') }}" class="sec-nav-item {{ request()->routeIs('super-admin.*') ? 'active' : '' }}">
            <i class="fas fa-server"></i> Super Admin (GEL)
          </a>
        </li>
        <li>
    ...
    @if($u)
      <div class="sec-sidebar-footer">
        ...
        <div style="flex:1;min-width:0;">
          <div class="sec-sidebar-footer-name">{{ $u->nom ?? $u->email }}</div>
          <div class="sec-sidebar-footer-role">{{ ucfirst($u->role ?? 'Utilisateur') }}</div>
        </div>
    ...
    <div id="aiWidgetPanel" class="ai-widget-panel shadow-lg">
      <div class="ai-widget-header">
        <div class="d-flex align-items-center gap-2">
          ...
          <div>
            <a href="/" class="sec-brand text-decoration-none" style="color:white;">
              <span style="font-weight:900;font-size:18px;letter-spacing:-1px;">GEL</span>
              <span style="font-weight:400;font-size:11px;opacity:0.8;margin-left:4px;">Assistant</span>
            </a>
          </div>
        </div>
