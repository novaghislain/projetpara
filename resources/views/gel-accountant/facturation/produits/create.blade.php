...
@section('title', 'Ajouter un Produit/Service')

@section('content')
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    <div class="mb-8">
        <h1 class="text-2xl md:text-3xl text-slate-800 dark:text-slate-100 font-bold">Ajouter un Produit/Service ✨</h1>
        <p class="text-sm text-slate-500">Ajoutez un nouvel élément à votre catalogue facturable.</p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
        <form action="{{ route('gel-accountant.facturation.produits.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nom" class="form-input w-full" value="{{ old('nom') }}" required placeholder="Ex: Mission d'Audit 2026" />
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0" name="prix_unitaire" class="form-input w-full" value="{{ old('prix_unitaire', 0) }}" required />
                </div>
                <div>
                    ...
                    <select name="est_service" class="form-select w-full">
                        <option value="1" {{ old('est_service') == '1' ? 'selected' : '' }}>Service (Prestation)</option>
                        <option value="0" {{ old('est_service') == '0' ? 'selected' : '' }}>Bien (Produit physique)</option>
                    </select>
                </div>
