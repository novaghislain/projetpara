...
@section('title', 'Commande de vente')

@section('content')

<div class="gel-page-header">
    <div>
        <h1 class="gel-page-title"><i class="fas fa-shopping-cart" style="color:var(--gel-primary); margin-right:8px;"></i> Créer une commande de vente</h1>
        <p class="gel-page-subtitle">Enregistrez la confirmation d'une vente avant livraison ou facturation.</p>
    </div>
...
<form method="POST" action="{{ route('gel-accountant.sales-order.store') }}" id="salesOrderForm">
@csrf
...
<div class="gel-card p-4 mb-4">
    <div class="doc-form-grid">
        ...
        <div class="doc-form-group">
            <label class="doc-label">Date de la commande *</label>
        ...
        <div class="doc-form-group">
            <label class="doc-label">NÂ° de commande</label>
            <input type="text" name="order_number" class="doc-input" value="SO-{{ str_pad(rand(1,9999), 4, '0', STR_PAD_LEFT) }}" readonly>
        </div>
    </div>
</div>
    ...
    <table class="doc-lines-table" id="linesTable">
        ...
        <tbody id="linesBody">
            <tr class="doc-line-row">
                ...
                <td><input type="text" name="lines[0][description]" class="doc-input-sm" placeholder="Nom du produit ou service..."></td>
...
<div style="display:grid; grid-template-columns:1fr 360px; gap:20px; margin-bottom:20px;">
    <div class="gel-card p-4 mb-4">
        <label class="doc-label">Message client sur la commande</label>
        <textarea name="message" class="doc-input" rows="3" placeholder="Nous vous remercions pour cette commande..."></textarea>
    </div>
    <div class="gel-card p-4 mb-4">
...
<div style="display:flex; justify-content:flex-end; gap:10px; padding-bottom:30px;">
    ...
    <button type="submit" class="gel-btn gel-btn-primary"><i class="fas fa-save"></i> Confirmer la commande</button>
</div>
</form>
...
<script>
let lineIndex=1;
function addLine(){const t=document.getElementById('linesBody');const r=document.createElement('tr');r.className='doc-line-row';r.innerHTML=`<td class="line-num">${t.children.length+1}</td><td><input type="text" name="lines[${lineIndex}][description]" class="doc-input-sm" placeholder="Nom du produit ou service..."></td><td><input type="number" name="lines[${lineIndex}][quantity]" class="doc-input-sm line-qty" value="1" min="1" step="0.01"></td><td><input type="number" name="lines[${lineIndex}][unit_price]" class="doc-input-sm line-price" value="0" min="0"></td><td><select name="lines[${lineIndex}][tax_rate]" class="doc-input-sm line-tax"><option value="0">0%</option><option value="18" selected>18%</option></select></td><td class="line-total" style="font-weight:600; text-align:right;">0 FCFA</td><td><button type="button" class="doc-line-remove" onclick="removeLine(this)"><i class="fas fa-times"></i></button></td>`;t.appendChild(r);lineIndex++;}
