...
<div class="gel-page-header">
    <div>
        <h1 class="gel-page-title"><i class="fas fa-sticky-note" style="color:var(--gel-primary); margin-right:8px;"></i> Émettre une Note de Crédit</h1>
        <p class="gel-page-subtitle">Créditez le compte d'un client suite À  un retour ou un trop-perçu.</p>
    </div>
...
<div class="gel-card p-4 mb-4">
    <div class="doc-form-grid">
        ...
        <div class="doc-form-group">
            ...
            <input type="date" name="date" class="doc-input" value="{{ date('Y-m-d') }}" required>
        </div>
        <div class="doc-form-group">
            <label class="doc-label">NÂ° Note de Crédit</label>
            <input type="text" name="credit_number" class="doc-input" value="CN-{{ str_pad(rand(1,9999), 4, '0', STR_PAD_LEFT) }}" readonly>
        </div>
    </div>
</div>
...
<div class="gel-card p-4 mb-4" style="overflow:hidden;">
    <table class="doc-lines-table" id="linesTable">
        <thead>
            <tr>
                <th style="width:5%;">#</th>
                <th style="width:30%;">Produit / Service</th>
                <th style="width:12%;">Quantité retournée</th>
        ...
        <tbody id="linesBody">
            <tr class="doc-line-row">
                ...
                <td><input type="text" name="lines[0][description]" class="doc-input-sm" placeholder="Raison ou produit..."></td>
...
<div style="display:grid; grid-template-columns:1fr 360px; gap:20px; margin-bottom:20px;">
    <div class="gel-card p-4 mb-4">
        <label class="doc-label">Message affiché sur la note de Crédit</label>
        <textarea name="message" class="doc-input" rows="3" placeholder="Suite À  votre demande..."></textarea>
    </div>
    <div class="gel-card p-4 mb-4">
        <div class="doc-summary-row"><span>Sous-total HT</span><span id="subtotal">0 FCFA</span></div>
        <div class="doc-summary-row"><span>TVA (À  récupérer)</span><span id="totalTax">0 FCFA</span></div>
        <div class="doc-summary-row doc-summary-total"><span>Total Crédit</span><span id="grandTotal">0 FCFA</span></div>
    </div>
</div>
...
<div style="display:flex; justify-content:flex-end; gap:10px; padding-bottom:30px;">
    ...
    <button type="submit" class="gel-btn gel-btn-primary"><i class="fas fa-check"></i> Créer la note de Crédit</button>
</div>
</form>
...
<script>
let lineIndex=1;
function addLine(){const t=document.getElementById('linesBody');const r=document.createElement('tr');r.className='doc-line-row';r.innerHTML=`<td class="line-num">${t.children.length+1}</td><td><input type="text" name="lines[${lineIndex}][description]" class="doc-input-sm" placeholder="Raison ou produit..."></td><td><input type="number" name="lines[${lineIndex}][quantity]" class="doc-input-sm line-qty" value="1" min="1" step="0.01"></td><td><input type="number" name="lines[${lineIndex}][unit_price]" class="doc-input-sm line-price" value="0" min="0"></td><td><select name="lines[${lineIndex}][tax_rate]" class="doc-input-sm line-tax"><option value="0">0%</option><option value="18" selected>18%</option></select></td><td class="line-total" style="font-weight:600; text-align:right;">0 FCFA</td><td><button type="button" class="doc-line-remove" onclick="removeLine(this)"><i class="fas fa-times"></i></button></td>`;t.appendChild(r);lineIndex++;}
