...
@section('title', 'Crédit de carte bancaire')

@section('content')

<div class="gel-page-header">
    <div>
        <h1 class="gel-page-title"><i class="fas fa-credit-card" style="color:var(--gel-primary); margin-right:8px;"></i> Crédit de carte bancaire</h1>
        <p class="gel-page-subtitle">Enregistrez un remboursement reçu sur la carte de Crédit de l'entreprise.</p>
    </div>
...
<form method="POST" action="#" id="creditCardCreditForm">
@csrf

<div class="gel-card p-4 mb-4">
    <div class="doc-form-grid">
        <div class="doc-form-group">
            <label class="doc-label">Bénéficiaire / Payeur</label>
        ...
        <div class="doc-form-group">
            <label class="doc-label">Compte de la carte de Crédit *</label>
...
<div class="gel-card p-4 mb-4" style="overflow:hidden;">
    <table class="doc-lines-table" id="linesTable">
        <thead>
            <tr>
                <th style="width:5%;">#</th>
                <th style="width:25%;">Compte / Catégorie de dépense affectée</th>
                <th style="width:35%;">Description</th>
                <th style="width:15%;">Montant remboursé</th>
        ...
        <tbody id="linesBody">
            <tr class="doc-line-row">
                ...
                <td><input type="text" name="lines[0][description]" class="doc-input-sm" placeholder="Ex: Remboursement vol annulé..."></td>
...
<div style="display:grid; grid-template-columns:1fr 360px; gap:20px; margin-bottom:20px;">
    <div class="gel-card p-4 mb-4">
        ...
        <textarea name="memo" class="doc-input" rows="3" placeholder="Explication du remboursement..."></textarea>
    </div>
    <div class="gel-card p-4 mb-4">
        <div class="doc-summary-row"><span>Sous-total HT</span><span id="subtotal">0 FCFA</span></div>
        <div class="doc-summary-row"><span>TVA récupérée</span><span id="totalTax">0 FCFA</span></div>
        <div class="doc-summary-row doc-summary-total"><span>Total Crédité sur la carte</span><span id="grandTotal">0 FCFA</span></div>
    </div>
</div>
...
<script>
let lineIndex=1;
function addLine(){const t=document.getElementById('linesBody');const r=document.createElement('tr');r.className='doc-line-row';r.innerHTML=`<td class="line-num">${t.children.length+1}</td><td><select name="lines[${lineIndex}][account_id]" class="doc-input-sm"><option value="">Catégorie...</option><option value="fournitures">Fournitures de bureau</option><option value="transport">Transport</option><option value="autre">Autre</option></select></td><td><input type="text" name="lines[${lineIndex}][description]" class="doc-input-sm" placeholder="Ex: Remboursement..."></td><td><input type="number" name="lines[${lineIndex}][amount]" class="doc-input-sm line-amount" value="0" min="0"></td><td><select name="lines[${lineIndex}][tax_rate]" class="doc-input-sm line-tax"><option value="0" selected>0%</option><option value="18">18%</option></select></td><td class="line-total" style="font-weight:600; text-align:right;">0</td><td><button type="button" class="doc-line-remove" onclick="removeLine(this)"><i class="fas fa-times"></i></button></td>`;t.appendChild(r);lineIndex++;}
