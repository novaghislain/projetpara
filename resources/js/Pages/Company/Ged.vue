...
const props = defineProps({
    pageTitle: { type: String, default: 'GED - Documents' }
});
...
async function deleteFolder(id) {
    try {
        ...
        alert(msg.message || 'Impossible de supprimer ce dossier.');
    }
}
...
function selectFolder(folder) {
    ...
    while (f) {
        ...
        f = null; // We need full path from API — for now just use current
    }
...
async function toggleArchive(doc) {
    try {
        const data = await apiPatch(`${apiBase}/documents/${doc.id}/archive`);
        await loadDocuments(currentFolder.value?.id || null);
    } catch (e) {
