...
namespace App\Http\Controllers\GelLegal\Auth;
...
class LegalRegisterController extends Controller
{
    ...
    {
        return view('gel-legal.auth.register-choices');
    }

    public function showAutonomousForm()
    {
        return view('gel-legal.auth.register-autonomous');
    }
    ...
    public function registerAutonomous(Request $request)
    {
        ...
        return DB::transaction(function () use ($validated) {
            $nomCabinet = $validated['personal_company_name'] ?? 'Cabinet Juridique â€” ' . $validated['name'];
            $entreprise = \App\Models\Entreprise::create([
                'raison_sociale' => $nomCabinet,
                'pays_code'      => 'BJ',
                'secteur_activite' => 'Juridique',
                'regime_fiscal'  => 'TPS',
                'modele_usage'   => 'full',
            ]);
            ...
            $role = \App\Models\Role::where('code', 'legal')->first();
            ...
            return redirect()->route('gel-legal.dashboard')
                ->with('success', 'Bienvenue dans votre espace Juridique !');
        });
    }
}